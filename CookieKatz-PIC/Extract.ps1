...
for ($i = 0; $i -lt $numberOfSections; $i++) {
    ...
    if ($name -eq ".text") {
        ...
        if ($CArray) {
            ...
            [void]$sb.AppendLine("unsigned char shellcode[$rawDataSize] = {")

            for ($j = 0; $j -lt $rawDataSize; $j++) {
                if ($j % 16 -eq 0) {
                    [void]$sb.Append("    ")
                }
            ...
            Write-Host "[+] Wrote C array ($rawDataSize bytes) to $outHeaderPath"
        }
        else {
            [System.IO.File]::WriteAllBytes($outPath, $shellcode)
            Write-Host "[+] Wrote $rawDataSize bytes to $outPath"
        }
        exit
    }
}
